...
class ParameterSet:
    ...
    def __init__(self, ps_len=0, ps_nalu=None):
        self.ps_len = ps_len
        self.ps_nalu = ps_nalu
    ...
    def __str__(self):
        logstr = "\t\t\t\t\t\t\t\t\t\t\tps_len = %08ld(0x%016lx)" \
                 "\t\t\t\t\t\t\t\t\t\t\tps_nalu = %08ld(0x%016lx)\n" % \
                 (self.ps_len, self.ps_len, self.ps_nalu, self.ps_nalu)
        return logstr
...
class SequenceParameterSet(ParameterSet):
    ...
    def __init__(self, sps_len=0, sps_nalu=None):
        ParameterSet.__init__(self, sps_len, sps_nalu)
...
class SequenceParameterSetExt(ParameterSet):
    ...
    def __init__(self, spse_len=0, spse_nalu=None):
        ParameterSet.__init__(self, spse_len, spse_nalu)
...
class AVCDecoderConfigurationRecord:
    ...
    class SequenceParameterSetExt:
        ...
        def decode(self, file_strm):
            ...
            self.num_spse = file_strm.ReadUInt8()
            self.offset += UInt8ByteLen
    ...
    def decode(self, file_strm):
        ...
        self.avc_profile_indication = file_strm.ReadUInt8()
        self.offset += UInt8ByteLen

        self.profile_compatibility = file_strm.ReadUInt8()
        self.offset += UInt8ByteLen

        self.avc_level_indication = file_strm.ReadUInt8()
        self.offset += UInt8ByteLen
        ...
        self.num_pps = file_strm.ReadUInt8()
        self.offset += UInt8ByteLen
    ...
    def dump(self):
        ...
        if self.num_pps > 0 and self.pps != None:
            ...
            for i in range(len(self.pps)):
                pps_ = self.pps[i]
                ppses['pps_%d' % i] = pps_[i].dump()
            dump_info['pps'] = ppses
        if self.spse != None:
            dump_info['spse'] = self.spse.dump()
        return dump_info
...
class AvcC(Box):
    ...
    def dump(self):
        ...
        if self.avcCRecord != None:
            dump_info[self.avcCRecord.type] = self.avcCRecord.dump()
        return dump_info
