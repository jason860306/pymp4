...
class MediaSegmentEntry:
    ...
    def decode(self, file_strm):
        ...
        if self.version == 1:
            self.segment_duration = file_strm.read_uint64()
            self.offset += UInt64ByteLen

            self.media_time = file_strm.read_int64()
            self.offset += Int64ByteLen

        else:
            self.segment_duration = file_strm.read_uint32()
            self.offset += UInt32ByteLen

            self.media_time = file_strm.read_int32()
            self.offset += Int32ByteLen

        self.media_rate_integer = file_strm.read_int16()
        self.offset += Int16ByteLen

        self.media_rate_fraction = file_strm.read_int16()
        self.offset += Int16ByteLen
