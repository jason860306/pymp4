...
class AudioSampleEntryV1(SampeEntry):
    ...
    def decode(self, file_strm):
        ...
        self.entry_version = file_strm.ReadUInt16()
        self.offset += UInt16ByteLen

        for i in range(len(self.reserved)):
            reserved_ = file_strm.ReadUInt16()
            self.reserved[i] = reserved_
            self.offset += UInt16ByteLen

        self.channelcount = file_strm.ReadUInt16()
        self.offset += UInt16ByteLen

        self.samplesize = file_strm.ReadUInt16()
        self.offset += UInt16ByteLen
        ...
        self.samplerate = file_strm.ReadUInt32()
        self.offset += UInt16ByteLen
